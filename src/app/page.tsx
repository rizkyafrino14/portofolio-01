...
const SOUND_CRACK = '/sounds/crack.mp3'
const SOUND_HATCH = '/sounds/hatch.mp3'
...
export default function Home() {
  ...
  const getImage = (crackValue: number) => {
    if (crackValue < 5) return '/egg.png'
    if (crackValue < 10) return '/hatch.png'
    return '/chicken.png'
  }
  ...
  const handleCrack = () => {
    ...
    const next = isCrack + 1
    setIsCrack(next)

    if (next === 10) {
      // Telur baru saja pecah — suara menetas
      playSound(SOUND_HATCH)
    } else if (next < 10) {
      // Masih dalam proses retak
      playSound(SOUND_CRACK)
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ScrambleText>Rizky Mustafa Afrino</ScrambleText>
